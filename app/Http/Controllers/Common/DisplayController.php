...
{ 
    ...
    {  
        ...
        if ($setting->display==6) // custom display
        {
            $display = DisplayCustom::where('company_id', auth()->user()->company_id)
                ->where('status', 1)
            ...
            if ($loop==1) 
            {
                $main = "<div class=\"col-sm-8\">
                    ...
                    <div class=\"queue well text-center ".(($loop==1)?'active':null)." \" style=\"height:auto;padding:160px 0;margin:0;font-size:36px\"> 
                        ...
                        <h2 class=\"token\" style=\"font-size:60px\">COUNTER: $display->counter</h2>
                    </div>
                </div>";
                ...
                $result .= "<div class=\"col-sm-12\"> 
                    <div class=\"queue well text-center \" style=\"background-color:".(!empty($setting->background_color)?$setting->background_color:'#cdcdcd') .";border-color:".(!empty($setting->border_color)?$setting->border_color:'#fff') .";min-height:75px;height:{$height}px;padding:0;margin:0 0 5px 0\">
                        ...
                        <div class=\"text-center\" style=\"display:inline-block;padding:10px 0\">
                            ...
                            <h3 class=\"token\"><b>COUNTER: $display->counter</b></h3>
                        </div>
                    </div>
                </div>";
            }
    ...
    { 
        ...
        $cTokens    = [];
        $company_id = auth()->user()->company_id;
        $customDisplay = DisplayCustom::find($request->customDisplayId);
                ...
                foreach ($value as $html2): 
                    ...
                    $html .=  "<div class=\"queue2 ".(($x==$sl)?'active':null)." \" style='height:{$height}px;background-color:".(!empty($setting->background_color)?$setting->background_color:'#cdcdcd') .";border-color:".(!empty($setting->border_color)?$setting->border_color:'#fff') .";color:".(!empty($setting->color)?$setting->color:'#cdcdcd') .";'>";
                        foreach ($html2 as $key => $item):
                            ...
                            if ($key=='token')
                            {
                                $html .=  "<h1 class=\"title\" style=\"font-size: 30px\">$item</h1>";
                            }
                            else
                            {
                                if ($setting->show_note == "1" && $key=='note')
                                {
                                    $html .=  "<h4 class=\"m-1\"><strong>".trans("app.note")."</strong>: <span>$item</span></h4>";
                                }
                                if ($setting->sms_alert == "1" && $key=='mobile')
                                {
                                    $html .=  "<h4 class=\"m-1\"><strong>".trans("app.mobile")."</strong>: <span> ".str_replace(range(0,9), "*",$item).substr($item,-4)." </span></h4>";
                                }
                                if ($setting->show_department == "1" && $key=='department')
                                {
                                    $html .=  "<h4 class=\"m-1\"><strong>".trans("app.department")."</strong>: <span>$item</span></h4>";
                                }
                                if($key=='section')
                                {
                                ...
                                {
                                    $html .=  "<h4 class=\"m-1\"><strong>".trans("app.officer")."</strong>: <span>$item</span></h4>";
                                }
                            }
                        endforeach;
                        $html .=  "</div>";
    ...
    // single line queue by location wise
    public function selectLocation()
    {
        ...
        return view('backend.common.display.locations', compact('departmentList',));
    }
    ...
    public function displayShow(Request $request)
    {
        ...
        return view('backend.common.display.single-line-queue', compact('setting'));
    }


    // Single Line Qeue
    public function display9(Request $request)
    {  
        // $company_id = 1;
        $company_id = auth()->user()->company_id;
