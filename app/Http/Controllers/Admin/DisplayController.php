...
{ 
    ...
    { 
        ...
        $counters = Counter::where('company_id', auth()->user()->company_id)
                    ->where('status', 1)->pluck('name', 'id'); 
        $customDisplays = DisplayCustom::where('company_id', auth()->user()->company_id)->get();  
            ...
            if (!empty($request->id)) 
            {
                ...
                $update = DisplaySetting::where('id',$request->id)
                    ->update([
                        ...
                        'alert_position'  => $request->alert_position ?? 1,
                    ]);
    ...
    {   
        ...
        {   
            ...
            {    
                ...
                $customDisplays = DisplayCustom::where('company_id', auth()->user()->company_id)
                                ->where('status', 1)->orderBy('name', 'ASC')->pluck('name', 'id');
