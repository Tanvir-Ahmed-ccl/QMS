...
{ 
    ...
    public function userData(Request $request) 
    {
        ...
        $totalData = User::where('id', '!=', auth()->id())->where('company_id', auth()->user()->company_id)->count();
        ...
        { 
            $usersProccess = User::where('company_id', auth()->user()->company_id)
                ->where('user_type', '!=', 5)
                ...
                if (!empty($search['user_type'])) {
                    $query->where('user_role_id', '=', $search['user_type']);
                }
    ...
    { 
        ...
        if ($validator->fails()) {
        	...
        	$save = User::insert([ 
                ...
                'user_type'   => 1,
                'user_role_id'   => $request->user_type,
                'photo'       => $filePath,
        ...
        if ($validator->fails()) { 
            ...
            $update = User::where('id',$request->id)
                ->update([  
                    ...
                    'status'      => $request->status,
                    'avg_token_completed_time'  => $request->avg_time,
                ]);
