...
class SmsSettingController extends Controller
{
    ...
    public function smsData(Request $request) 
    {
        ...
        $totalData = SmsHistory::where('company_id', auth()->user()->company_id)->count();
        ...
        if(empty($search))
        {            
            $smses = SmsHistory::where('company_id', auth()->user()->company_id)
                    ->offset($start)
                    ->limit($limit)
                    ->orderBy($order,$dir)
                    ->get();
        ...
        { 
            $smsesProccess = SmsHistory::where('company_id', auth()->user()->company_id)
        ...
        if(!empty($smses))
        {
            ...
            $data[] = [
                'serial'     => $loop++,
                'to'         => $sms->to,
                'message'    => substr_replace($sms->message, '...', 100),
    ...
    {
    	$sms_setting = SmsSetting::where('company_id', auth()->user()->company_id)->first();
    ...
    public function sendFromTwilio(Request $request)
    {
        @date_default_timezone_set(session('app.timezone'));
        ...
        {     
        	$twilio = AppSettings::first();
            $companyId = Auth::user()->company_id;
            $planId = Auth::user()->company->subscription_plan;
            $smsLimit = Auth::user()->company->sms_limit;
            ...
            if($totalSMS = SmsHistory::where('company_id', $companyId)->count() >= $smsLimit)
            {
                if($totalSMS == 0)
                $msg = "14 days trial account cannot send sms";
                else
                $msg = "Sms Limit is over. Please purchase a new plan";
