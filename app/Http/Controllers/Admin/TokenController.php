...
class TokenController extends Controller
{
    ...
    { 
        ...
        $departmentList = Department::where('company_id', auth()->user()->company_id)->where('status',1)->pluck('name','id');
        $sections = Section::where('company_id', auth()->user()->company_id)->where('status',1)->pluck('name','id');
        $userList = User::select('id', DB::raw('CONCAT(firstname, " ", lastname, " <", email, ">") AS name'))->where('company_id', auth()->user()->company_id)->where('user_type',1)->where('status',1)->orderBy('name', 'ASC')->pluck('name', 'id');

        return view('backend.admin.token.setting', compact('tokens','countertList','departmentList','userList', 'sections')); 
    }
    ...
    {
        $display = DisplaySetting::where('company_id', auth()->user()->company_id)->first();
        ...
        else
        {
            $departmentList = TokenSetting::select( 
                ...
                ->join('counter', 'counter.id', '=', 'token_setting.counter_id')
                ->join('user', 'user.id', '=', 'token_setting.user_id')
                ->where('department.company_id', auth()->user()->company_id)
                ->where('token_setting.status', 1)
    ...
    public function autoToken(Request $request)
    {   
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                $saveToken = [
                    ...
                    'token_no'      => (new Token_lib)->newToken($settings['department_id'], $settings['counter_id']),
                    'client_mobile' => $request->client_mobile,
    ...
    public function reportData(Request $request)
    {
        ...
        $totalData = Token::whereHas('department', function($q){
                            $q->where('company_id', auth()->user()->company_id);
                        })->count();
        ...
        { 
            $tokensProccess = Token::whereHas('department', function($q){
                                $q->where('company_id', auth()->user()->company_id);
                            })
    ...
    { 
        ...
        $tokens = DB::select("
         SELECT 
          ...
          (SELECT CONCAT_WS(' ', firstname, lastname) FROM user WHERE id= realToken.user_id AND company_id = $company_id) as officer,
          (
            SELECT COUNT(id) 
            FROM token 
            WHERE 
          ...
          FROM 
            token AS realToken WHERE company_id = $company_id
          GROUP BY user_id
        ");
    ...
    public function recall($id = null)
    {
        ...
        $setting  = SmsSetting::where('company_id', auth()->user()->company_id)->first(); 
